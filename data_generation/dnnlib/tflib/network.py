...
class Network:
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=True), tf.compat.v1.name_scope(self.name):
            ...
            for expr, name, shape in zip(in_expr, self.input_names, self.input_shapes):
                if expr is not None:
                    ...
                    expr = tf.compat.v1.zeros([tf.compat.v1.shape(valid_inputs[0])[0]] + shape[1:], name=name)
                final_inputs.append(expr)
            out_expr = self._build_func(*final_inputs, **build_kwargs)

        # Propagate input shapes back to the user-specified expressions.
        for expr, final in zip(in_expr, final_inputs):
            if isinstance(expr, tf.compat.v1.Tensor):
    ...
    def setup_as_moving_average_of(self, src_net: "Network", beta: TfExpressionEx = 0.99, beta_nontrainable: TfExpressionEx = 0.0) -> tf.compat.v1.Operation:
        ...
        if key not in self._run_cache:
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.compat.v1.control_dependencies(None):
                with tf.compat.v1.device("/cpu:0"):
                    in_expr = [tf.compat.v1.placeholder(tf.compat.v1.float32, name=name) for name in self.input_names]
                    in_split = list(zip(*[tf.compat.v1.split(x, num_gpus) for x in in_expr]))

                out_split = []
                for gpu in range(num_gpus):
                    with tf.compat.v1.device("/gpu:%d" % gpu):
    ...
    def setup_weight_histograms(self, title: str = None) -> None:
        ...
        with tf.compat.v1.name_scope(None), tf.compat.v1.device(None), tf.compat.v1.control_dependencies(None):
...
def _legacy_output_transform_func(*expr, out_mul=1.0, out_add=0.0, out_shrink=1, out_dtype=None):
    ...
    if out_shrink > 1:
        ...
        expr = [tf.compat.v1.nn.avg_pool(x, ksize=ksize, strides=ksize, padding="VALID", data_format="NCHW") for x in expr]

    if out_dtype is not None:
        if tf.compat.v1.as_dtype(out_dtype).is_integer:
            expr = [tf.compat.v1.round(x) for x in expr]
        expr = [tf.compat.v1.saturate_cast(x, out_dtype) for x in expr]
    return expr
